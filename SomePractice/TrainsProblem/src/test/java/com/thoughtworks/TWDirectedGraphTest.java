...
public class TWDirectedGraphTest {
    ...
    public void testShortestPathBetween2() {
        ...
        try {
            ...
            assertEquals(Optional.of(6), Optional.ofNullable(path.distance()));
            assertEquals(Optional.of(2), Optional.ofNullable(path.stationCount()));

            path = graph.shortestPathBetween(towns.get("E"), towns.get("C"));
            assertEquals("E-->B-->C", path.toString());
            assertEquals(Optional.of(7), Optional.ofNullable(path.distance()));
            assertEquals(Optional.of(2), Optional.ofNullable(path.stationCount()));

            path = graph.shortestPathBetween(towns.get("D"), towns.get("B"));
            assertEquals("D-->E-->B", path.toString());
            assertEquals(Optional.of(9), Optional.ofNullable(path.distance()));
            assertEquals(Optional.of(2), Optional.ofNullable(path.stationCount()));
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage());
        }
    ...
    public void testCountRoutesWithHops3() throws TWDirectedGraph.NoSuchNodeException {
        int distance = graph.numOfRouteWithMaxDistance(towns.get("B"), towns.get("C"), 1);
        assertEquals(1, distance);
    }
